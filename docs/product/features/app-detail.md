...
parsing the APK signing block, which `FR-36` has now landed: `ApkSigningBlockAnalyzer` in
`core:apps` reads the APK Signing Block directly for v2/v3/v3.1 and scans `META-INF/` for a legacy
v1 JAR signature. What is free from `SigningInfo` regardless — `hasMultipleSigners()` and
`hasPastSigningCertificates()`, the latter meaning the signing key was rotated — backs the "Current
signers" fact next to the SCHEME row. Version detection is defensive: anything structurally
