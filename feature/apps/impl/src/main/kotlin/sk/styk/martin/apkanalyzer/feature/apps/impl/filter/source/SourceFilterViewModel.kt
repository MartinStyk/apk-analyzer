...
class SourceFilterViewModel @Inject constructor(
    ...
    private val selectedSources = MutableStateFlow(input.selectedSources.toSet())
    ...
    fun onAction(action: SourceFilterAction) {
        when (action) {
            is SourceFilterAction.SourceToggled -> selectedSources.update { current ->
                if (action.source in current) current - action.source else current + action.source
            }

            SourceFilterAction.Reset -> selectedSources.value = setOf()
